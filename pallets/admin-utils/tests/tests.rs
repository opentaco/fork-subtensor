...
#[test]
fn test_sudo_set_lambda() {
    new_test_ext().execute_with(|| {
        let netuid: u16 = 1;
        let to_be_set: u16 = 10;
        add_network(netuid, 10);
        let init_value: u16 = SubtensorModule::get_lambda(netuid);
        assert_eq!(
            AdminUtils::sudo_set_lambda(
                <<Test as Config>::RuntimeOrigin>::signed(U256::from(1)),
                netuid,
                to_be_set
            ),
        ...
        assert_eq!(SubtensorModule::get_lambda(netuid), init_value);
        assert_ok!(AdminUtils::sudo_set_lambda(
            <<Test as Config>::RuntimeOrigin>::root(),
            netuid,
            to_be_set
        ));
        assert_eq!(SubtensorModule::get_lambda(netuid), to_be_set);
    });
}
...
fn test_sudo_set_mu() {
    new_test_ext().execute_with(|| {
        ...
        let to_be_set: u16 = 10;
        add_network(netuid, 10);
        let init_value: u16 = SubtensorModule::get_mu(netuid);
        assert_eq!(
            AdminUtils::sudo_set_mu(
                <<Test as Config>::RuntimeOrigin>::signed(U256::from(1)),
                netuid,
                to_be_set
            ),
        ...
        assert_eq!(SubtensorModule::get_mu(netuid), init_value);
        assert_ok!(AdminUtils::sudo_set_mu(
            <<Test as Config>::RuntimeOrigin>::root(),
            netuid,
            to_be_set
        ));
        assert_eq!(SubtensorModule::get_mu(netuid), to_be_set);
    });
}
